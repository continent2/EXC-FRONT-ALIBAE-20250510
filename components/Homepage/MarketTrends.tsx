...
//  let [ market_trend_title , set_market_trend_title ] = useState ( 'You can trade a variety of cryptocurrencies.')
  let [ market_trend_title , set_market_trend_title ] = useState ( landing?.market_trend_title )  
const gethrefforpair=( routerlocale:   Nullable<string>  , child_coin_name : string , parent_coin_name : string ): string =>{ // return ''
  return  routerlocale !== "en"
  ? `/${
      routerlocale
    }/exchange/dashboard?coin_pair=${
      child_coin_name + // item?.
      "_" +
      parent_coin_name // item?.
    }`
  : `/exchange/dashboard?coin_pair=${
      child_coin_name + // item?.
      "_" +
          ...
          <div className="tradex-container tradex-space-y-6 md:tradex-space-y-10">
            <div className=" tradex-flex tradex-justify-between tradex-items-center">
              <h3 className=" tradex-text-[24px] tradex-leading-[32px] md:tradex-text-[32px] md:tradex-leading-[48px] xl:tradex-text-[48px] xl:tradex-leading-[60px] !tradex-text-title tradex-font-bold">
                ...
                {/* { t( landing?.market_trend_title )  || t("Crypto Market Trend")} */}
                { t( "You can trade a variety of cryptocurrencies." )  || t("Crypto Market Trend")}
              </h3>
              <Link href={`/markets`}>
                    ...
                    <table className=" tradex-w-full tradex-border-separate tradex-border-spacing-y-2">
                      ...
                      <tbody>
                        {localAssetCoinPairs?.map((item: any, index: any) => (
                          ...
                          >
{/* <Link href={ gethrefforpair( router.locale , item?.child_coin_name , item?.parent_coin_name ) } >
                            <a> */}

                        {/* <Link href={ gethrefforpair( router.locale , item?.child_coin_name , item?.parent_coin_name ) } > */}
                            {/***** THE FIRST */}
                            {/* <Link href={ gethrefforpair( router.locale , item?.child_coin_name , item?.parent_coin_name ) } >
                            <a className="tradex-py-3 tradex-px-6 tradex-rounded-lg group-hover:tradex-bg-primary group-hover:!tradex-text-white !tradex-text-title tradex-bg-transparent tradex-text-sm tradex-leading-[18px]"> */}
                            {/* <a> */}
                            <td
                              className={` ${
                            ...
                            >
                              <p className="tradex-text-base tradex-font-medium !tradex-text-title" style={{ textAlign : 'center' , display:'inline-block' , margin:'0 auto' }}>
                                $
                                { item.last_price }
                                {/* {formatCurrency(
                                  item.last_price,
                            ...
                            >
                              <p
                                className={`tradex-text-base tradex-font-medium tradex-text-primary ${
